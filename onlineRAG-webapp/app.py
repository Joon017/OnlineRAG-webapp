...
def initialise(): #Initialising everything
    ...
    try:
        ...
        print("User's vectordb initialized")
...
def query_chain():
    try:
        ...
        for doc in docs:
            ...
            print(doc_details['metadata']['source'])
            print(doc_details['page_content'])
            source = "Source filename: " + str(doc_details['metadata']['source'] + "\n\n\n")
            content = str(doc_details['page_content'])
            sources_combined = source + "\n\n" + str(content)
            sources[source_index] = content
            source_index += 1
        
        print(sources_combined)

        query = f"""Given the question: {query},
